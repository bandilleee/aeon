    ...
    public class EmailService
    {
        ...
        private async Task SendAsync(string toEmail, string toName, string subject, string htmlBody)
        {
            try
            {
                ...
                var gmailUser = _config["Gmail:Username"]   ?? FROM_EMAIL;
                var gmailPass = _config["Gmail:AppPassword"] 
                    ?? throw new InvalidOperationException("Gmail:AppPassword is not configured in appsettings.");
            ...
            {
                // Never crash the API over email failure
